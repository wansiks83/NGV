---
name: tdd
description: Python unittest 기반 테스트 주도 개발(TDD) 방법론을 제공합니다. obra/superpowers의 test-driven-development 스킬을 이 프로젝트(Python 3.14, unittest, A-SPICE 상세설계 기반 구현)에 맞게 재구성했습니다. 기능/버그수정 구현 전 실패하는 테스트를 먼저 작성하고, 최소 구현으로 통과시키고, 품질지표를 지키며 리팩터링하는 절차와, 테스트 함수에 기법·긍정/부정 케이스·목적을 명시하는 Doxygen 주석 규칙이 필요할 때 사용하세요. 시험 케이스 도출 기법 자체의 정의는 `test-design-techniques` 스킬을 따릅니다. `coding` 서브에이전트가 구현 단계에서 항상 이 스킬을 사용합니다.
---
...
코드를 작성하기 전에, 상세설계서(`detailed-designer` 스킬의 산출물)에서 테스트 케이스와 각 케이스가 사용할 **시험 기법**을 함께 도출한다. 도출한 목록은 테스트 코드를 작성하기 전에 사용자에게 먼저 요약해 보여준다. 아래 표의 "시험 기법" 열은 이름만 나열한 것이며, 각 기법의 정의와 적용 방법은 공용 스킬 `test-design-techniques`를 참고한다(중복 정의하지 않는다).
...
- `@technique`: 0단계에서 도출한 시험 기법 중 해당하는 것을 명시한다. 기법 목록과 정의는 공용 스킬 `test-design-techniques`의 카탈로그를 따른다. 여러 기법이 섞여 있으면 모두 나열한다.
